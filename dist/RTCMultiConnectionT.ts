...
declare const VideoStreamTrack: any;
declare const AudioStreamTrack: any;
declare const FileBufferReader: any;
declare const define: any;
declare const cordova: any;
declare const DetectRTC: any;
declare const adapter: any;
declare const mozRTCPeerConnection: any;
declare const hark: any;
declare const webkitMediaStream: any;

export const RTCMultiConnection = (roomid, forceOptions) => {
    const isNodejs = typeof process === 'object' && typeof process.versions === 'object' && process.versions.node && /*node-process*/ !process['browser'];
    ...
    function MultiPeers(connection) {
        ...
        this.shareFile = function(file, remoteUserId) {
            ...
            connection.fbr.readAsArrayBuffer(file, function(uuid) {
                ...
                chunkSize: window['DetectRTC'].browser.name === 'Firefox' ? 15 * 1000 : connection.chunkSize || 0
            });
        };

        const textReceiver = new TextReceiver(connection);
    ...
    (function() {
        ...
        const navigator: Navigator & {enumerateDevices: (cb) => MediaDeviceInfo[]} = window.navigator as any;
        ...
        function getBrowserInfo() {
            ...
            let majorVersion: string|number = parseInt(navigator.appVersion, 10);
            ...
            switch (os) {
                ...
                case 'iOS':
                    if (/OS (\d+)_(\d+)_?(\d+)?/.test(nAgt)) {
                        const m = /OS (\d+)_(\d+)_?(\d+)?/.exec(nVer);
                        osVersion = m[1] + '.' + m[2] + '.' + (m[3] as any | 0);
                    }
                    break;
            }
        ...
        if (navigator.mediaDevices && navigator.mediaDevices.enumerateDevices) {
            ...
            navigator['enumerateDevices'] = function(callback): /*Promise<MediaDeviceInfo[]>*/ any {
                navigator.mediaDevices.enumerateDevices()
                    .then(o => callback ? callback(o): o )
                    .catch(function() {
                    callback([]);
                });
            };
        ...
        function checkDeviceSupport(callback) {
            ...
            navigator.enumerateDevices((devices: MediaDeviceInfo[]) => {
                devices.forEach((_device: MediaDeviceInfo) => {
                    let device: MediaDeviceInfo = {} as any;
                    ...
                    if (device.kind === 'audio' as any) {
                        (device as any)['kind'] = 'audioinput';
                    }

                    if (device.kind === 'video' as any) {
                        (device as any)['kind'] ='videoinput';
                    }

                    if (!device.deviceId) {
                        (device as any)['deviceId'] = device['id'];
                    }
                    ...
                    if (!device.label) {
                        (device as any)['label'] = 'Please invoke getUserMedia once.';
                        if (typeof DetectRTC !== 'undefined' && DetectRTC.browser.isChrome && DetectRTC.browser.version >= 46 && !/^(https:|chrome-extension:)$/g.test(location.protocol || '')) {
                            if (typeof document !== 'undefined' && typeof document.domain === 'string' && document.domain.search && document.domain.search(/localhost|127.0./g) === -1) {
                                (device as any)['label'] = 'HTTPs is required to get label of this ' + device.kind + ' device.';
                            }
                        }
                    } else {
        ...
        // ------
        let isGetUserMediaSupported: boolean|string = false;
        ...
        if (DetectRTC.browser.isFirefox && typeof window['mozRTCPeerConnection'] !== 'undefined' /*&& DetectRTC.browser.version > 39*/) {
            /*global mozRTCPeerConnection:true */
            if ('getSenders' in window['mozRTCPeerConnection'].prototype) {
                isRTPSenderReplaceTracksSupported = true;
            }
        } else if (DetectRTC.browser.isChrome && typeof webkitRTCPeerConnection !== 'undefined') {
            /*global webkitRTCPeerConnection:true */
            if ('getSenders' in window['webkitRTCPeerConnection'].prototype) {
                isRTPSenderReplaceTracksSupported = true;
            }
        }
    ...
    function fireEvent(obj, eventName, args) {
        ...
        const event = new CustomEvent(eventName, eventDetail as any);
    ...
    function setHarkEvents(connection, streamEvent) {
        ...
        hark(streamEvent.stream, {
            ...
            onvolumechange: function(volume, threshold) {
                ...
                connection.onvolumechange(Object.assign({
                    volume: volume,
                    threshold: threshold
                }, streamEvent));
            }
    ...
    function getRandomString() {
        if (window.crypto && window.crypto.getRandomValues && navigator.userAgent.indexOf('Safari') === -1) {
            const len = 3;
            const a = window.crypto.getRandomValues(new Uint32Array(len));
            let token = '';
            for (let i = 0; i < len; i++) {
    ...
    function getRMCMediaElement(stream, callback, connection) {
        ...
        if (DetectRTC.browser.name === 'Firefox') {
            ...
            mediaElement.addEventListener(streamEndedEvent, function() {
                ...
                window['currentUserMediaRequest'].remove(stream.idInstance);
    ...
    function isAudioPlusTab(connection, audioPlusTab) {
        ...
        if (typeof audioPlusTab === 'boolean' && audioPlusTab) {
            return true;
        }
    ...
    function setSdpConstraints(config) {
        ...
        if (oldBrowser) {
            sdpConstraints = {
                mandatory: sdpConstraints as any,
                optional: [{
    ...
    let MediaStreamTrack = window['MediaStreamTrack'];
    ...
    function PeerInitiator(config) {
        ...
        const renegotiatingPeer = !!config.renegotiatingPeer;
        if (config.remoteSdp) {
        ...
        function getLocalStreams() {
            ...
            if (typeof window.InstallTrigger !== 'undefined' && 'getSenders' in peer && typeof peer.getSenders === 'function') {
                const streamObject2 = new MediaStream();
        ...
        const incomingStreamEvent = 'track';
    ...
    const OnIceCandidateHandler = (function() {
        function processCandidates(connection, icePair) {
            const candidate = icePair.candidate;

            const iceRestrictions = connection.candidates;
            const stun = iceRestrictions.stun;
            const turn = iceRestrictions.turn;
    ...
    window.currentUserMediaRequest = {
        ...
        remove: function(idInstance) {
            ...
            const stream = this.streams[idInstance];
            if (!stream) {
                return;
            }
    ...
    function getUserMediaHandler(options) {
        if (window['currentUserMediaRequest'].mutex === true) {
            window['currentUserMediaRequest'].queueRequests.push(options);
            return;
        }
        window['currentUserMediaRequest'].mutex = true;
        ...
        function streaming(stream, returnBack) {
            ...
            stream.addEventListener(streamEndedEvent, function() {
                delete window['currentUserMediaRequest'].streams[idInstance];

                window['currentUserMediaRequest'].mutex = false;
                if (window['currentUserMediaRequest'].queueRequests.indexOf(options)) {
                    delete window['currentUserMediaRequest'].queueRequests[window['currentUserMediaRequest'].queueRequests.indexOf(options)];
                    window['currentUserMediaRequest'].queueRequests = removeNullEntries(window['currentUserMediaRequest'].queueRequests);
                }
            }, false);

            window['currentUserMediaRequest'].streams[idInstance] = {
                stream: stream
            };
            window['currentUserMediaRequest'].mutex = false;

            if (window['currentUserMediaRequest'].queueRequests.length) {
                getUserMediaHandler(window['currentUserMediaRequest'].queueRequests.shift());
            }
        }

        if (window['currentUserMediaRequest'].streams[idInstance]) {
            streaming(window['currentUserMediaRequest'].streams[idInstance].stream, true);
        } else {
            ...
            if (typeof navigator.mediaDevices === 'undefined') {
                navigator.getUserMedia = navigator.getUserMedia || navigator['webkitGetUserMedia'] || navigator.mozGetUserMedia;
                const getUserMediaSuccess = function() {};
                const getUserMediaFailure = function() {};

                const getUserMediaStream, getUserMediaError;
    ...
    // global constiables
    const chromeMediaSource = 'screen';
    const sourceId;
    ...
    function isFirefoxExtensionAvailable(callback) {
        ...
        const isFirefoxAddonResponded = false;

        function messageCallback(event) {
            const addonMessage = event.data;
    ...
    function TextReceiver(connection): void {
        const content = {};
        ...
        function receive(data, userid, extra) {
            ...
            if (data.last) {
                const message = content[uuid].join('');
                if (data.isobject) {
    ...
    const FileProgressBarHandler = (function() {
        function handle(connection) {
            ...
            connection.onFileEnd = function(file) {
                const helper = progressHelper[file.uuid];
                if (!helper) {
    ...
    (function(connection) {
        ...
        connection.openOrJoin = function(localUserid, password) {
            connection.checkPresence(localUserid, function(isRoomExists, roomid) {
                ...
                if (isRoomExists) {
                    ...
                    const localPeerSdpConstraints = false;
                    const remotePeerSdpConstraints = false;
        ...
        function beforeJoin(userPreferences, callback) {
            ...
            const session = userPreferences.session || connection.session;
        ...
        if (!forceOptions.useDefaultDevices && !DetectRTC.isMobileDevice) {
            DetectRTC.load(function() {
                const lastAudioDevice, lastVideoDevice;
        ...
        function keepNextBroadcasterOnServer() {
            ...
            const firstBroadcaster = connection.broadcasters[0];
            const otherBroadcasters = [];
            connection.broadcasters.forEach(function(broadcaster) {
        ...
        connection.streamEvents = {
            selectFirst: function(options) {
                if (!options) {
                    ...
                    const firstStream;
                    for (let str in connection.streamEvents) {
